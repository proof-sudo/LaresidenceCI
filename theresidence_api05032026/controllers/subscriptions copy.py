...
class SubscriptionsController(http.Controller):
    ...
    def list_subscriptions(self, **kwargs):
        ...
        domain = [('x_tr_is_subscription', '=', True)]
        if kwargs.get('memberId'):
            domain.append(('partner_id.x_tr_uuid', '=', kwargs['memberId']))
        if kwargs.get('status'):
            domain.append(('x_tr_subscription_status', '=', kwargs['status']))
        ...
        subscriptions = request.env['sale.order'].sudo().search(domain, offset=page * size, limit=size, order='create_date desc')
