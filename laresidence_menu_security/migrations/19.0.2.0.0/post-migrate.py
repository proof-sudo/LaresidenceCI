...
def migrate(cr, version):
    """Restreint le menu booking_engine (module tiers) à erp_manager."""
    cr.execute("""
    ...
    if menu_ids:
        _logger.info("migrate: %d menu(s) booking_engine restreint(s) à erp_manager", len(menu_ids))
    else:
        _logger.info("migrate: aucun menu booking_engine trouvé")
